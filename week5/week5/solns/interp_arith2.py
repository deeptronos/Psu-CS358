...
type Expr = Add | Sub | Mul | Div | Neg | Lit | Let | Name | Exp
...
@dataclass
class Exp():  # New class for exponentiation
    left: Expr
    right: Expr
    def __str__(self) -> str:
        return f"({self.left} ^ {self.right})"
...
def evalInEnv(env: Env[int], e:Expr) -> int:
    match e:
        ...
        case Exp(l,r):  # Handle exponentiation
            lv = evalInEnv(env,l)
            rv = evalInEnv(env,r)
            if rv < 0:
                raise EvalError("exponent must be non-negative")
            return lv ** rv
